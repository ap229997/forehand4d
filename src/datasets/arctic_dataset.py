...
class ArcticDataset(BaseDataset):
    ...
    def getitem(self, imgname, load_rgb=True):
        ...
        data_params = seq_data["params"] # world2ego can be used to convert 3D points to world frame
        ...
        # self.aug_data = False # overfitting only works without augmentation, comment this for training
        # args.use_gt_k = True
    ...
    def _load_data(self, args, split, seq):
        ...
        data_p = op.join(
            f"{os.environ['DOWNLOADS_DIR']}/data/arctic/data/splits/{args.setup}_{short_split}.npy"
        )
