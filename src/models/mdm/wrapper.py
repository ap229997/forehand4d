...
from src.models.mdm.model_ import MotionDiffusion
...
class MotionDiffusionWrapper(GenericWrapper):
    def __init__(self, args, push_images_fn):
        ...
        if 'future_j2d' in args.get('cond_mode', 'no_cond') or args.get('finetune_2d', 0) == 1:
            # lifting model doesn't work without 2d loss
    ...
    def forward(self, inputs, targets, meta_info, mode):
        ...
        if hasattr(self, "motion_data") and self.motion_data:
            pred = self.model(inputs, meta_info, targets)
        else:
        ...
        # add diffusion losses with weight 1
        for k, v in pred.items():
            if 'diff_mse' in k:
                loss_dict[f"loss/{k}"] = (v.mean(), 1.0)
    ...
    def inference(self, inputs, targets, meta_info, mode='vis', num_samples=1):
        models = {
            "mano_r": self.mano_r,
            "mano_l": self.mano_l
        }
    ...
    def refine_model_predictions(self, inputs, targets, meta_info, mode='val', return_metrics=False):
        models = {
            "mano_r": self.mano_r,
            "mano_l": self.mano_l
        }
        ...
        out = optimize_mano_params(inputs, meta_info, targets, self.model, self.args)
        pose_r, transl_r, pose_l, transl_l = out

        if return_metrics:
            ############### this is for computing metrics ################
            bz, ts = pose_r.shape[:2]
            K = meta_info['intrinsics'][:, -1:].repeat(1, ts, 1, 1)
            mano_output_r, mano_output_l = self.model.run_mano_on_pose_predictions(pose_r, transl_r, pose_l, transl_l, targets, K)
        ...
        if return_metrics:
            return pose_r, transl_r, pose_l, transl_l, metrics_all
        return pose_r, transl_r, pose_l, transl_l