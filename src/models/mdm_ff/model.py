...
class MotionFeedForward(nn.Module):
    def __init__(self, focal_length, img_res, args):
        super().__init__()
        self.args = args
        
        # only architecture params are used so that model is exactly the same as others
        self.model, _ = create_model_and_diffusion(args, data=None)
        ...
        assert self.cond_mode != 'no_cond', 'Conditioning mode must be specified'
    ...
    def forward(self, inputs, meta_info, targets=None, **kwargs):
        ...
        motion_out = self.forward_pass(cond['y'])
    ...
    # naming is done to be consistent with mdm wrapper, only forward pass is called here
    ...
    def forward_pass(self, y):
        ...
        if 'text' in self.cond_mode:
            if 'text_embed' in y.keys():  # caching option
                ...
                enc_text = self.model.encode_text(y['text'])
            emb.append(self.model.embed_text(self.model.mask_cond(enc_text, force_mask=force_mask)))
        if 'action' in self.cond_mode:
            action_emb = self.model.embed_action(y['action'])
            emb.append(self.model.mask_cond(action_emb, force_mask=force_mask))
        if 'pose' in self.cond_mode or 'img' in self.cond_mode:
            emb.append(self.model.mask_cond(y['joint'], force_mask=force_mask))
        if 'spatial' in self.cond_mode:
            # concatenate emb and y['spatial']
            spatial_emb = self.model.mask_spatial(y['spatial'], force_mask=force_mask) # [bs, spatial_dim, d]
        ...
        if self.model.arch == 'trans_enc':
            ...
            emb_dim = emb.shape[0]
            output = self.model.seqTransEncoder(xseq)[emb_dim:]
        else:
            raise NotImplementedError

        output = self.model.output_process(output)
        return output
