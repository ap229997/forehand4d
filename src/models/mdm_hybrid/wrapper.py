...
from src.callbacks.loss.loss_function import compute_loss_motion, compute_loss_motion_hybrid, compute_loss_2d_hybrid
...
from src.models.mdm_hybrid.model import MotionDiffusionHybrid
...
class MotionDiffusionHybridWrapper(GenericWrapper):
    def __init__(self, args, push_images_fn):
        super().__init__(args, push_images_fn)

        self.args = args
        self.model = MotionDiffusionHybrid(
        ...
        self.loss_fn = [compute_loss_motion, compute_loss_motion_hybrid]
        self.metric_dict = [
            "mpjpe.future",
            "mrrpe.future",
        ]
        ...
        if 'assembly' in dataset or 'holo' in dataset or 'epic' in dataset:
            self.loss_fn = [compute_loss_motion, compute_loss_motion_hybrid, compute_loss_2d_hybrid]
