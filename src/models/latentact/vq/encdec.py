...
from src.models.latentact.vq.resnet import Resnet1D
from src.models.latentact.vq.transformer import TransformerEncoderLayer, TransformerDecoderLayer, TransformerEncoder, TransformerDecoder
...
class TFEncoder(nn.Module):
    def __init__(self, opt) -> None:
        ...
        self.inp_head = nn.Sequential(nn.Linear(inp_pose, opt.code_dim), nn.ReLU())
        self.enc = TransformerEncoderLayer(d_model=opt.code_dim, nhead=opt.vqvae_tfenc_nhead, batch_first=True, dropout=opt.vqvae_tfenc_dp)
        self.transformer = TransformerEncoder(self.enc, num_layers=opt.vqvae_tfenc_nlayer)
...
class TFDecoder(nn.Module):
    def __init__(self, opt) -> None:
        ...
        self.dec = TransformerDecoderLayer(d_model=opt.tf_dim, nhead=opt.vqvae_tfdec_nhead, batch_first=True, dropout=opt.vqvae_tfdec_dp)
        self.transformer = TransformerDecoder(self.dec, num_layers=opt.vqvae_tfdec_nhead)
        ...
        pos_enc = nn.Parameter(torch.randn(opt.max_motion_length, opt.tf_dim))
...
class ContactModule(nn.Module):
    def __init__(self, opt) -> None:
        ...
        # keep both grid_conv and contact_conv of the same dimension for now
...
class JointConditioner(nn.Module):
    def __init__(self, opt) -> None:
        super().__init__()
        self.opt = opt
        assert self.opt.diffusion # only supports diffusion training
        ...
        self.joint_mlp = nn.Sequential(
                nn.Linear(joint_dim, opt.latent_dim),
                nn.ReLU(),
            )

        # 1D CNN along the time axis
        downsample_resolution = [2, 2, 2, 2]
        self.time_conv = nn.ModuleList()
        for i in range(len(downsample_resolution)):
            self.time_conv.append(nn.Conv1d(opt.latent_dim, opt.latent_dim, 3, downsample_resolution[i], 1))
            self.time_conv.append(nn.ReLU())
    ...
    def forward(self, x, **kwargs):
        ...
        # 1D CNN along the time axis
        memory = memory.permute(0, 2, 1) # (B, latent_dim, T)
        for module in self.time_conv:
            memory = module(memory)
        memory = memory.mean(dim=-1) # (B, latent_dim)
...
class FeedForward(nn.Module):
    ...
    def forward(self, x=None, **kwargs) -> torch.Tensor:
        ...
        if self.opt.use_vit:
            ...
            video_feats = video_feats.repeat(1, self.opt.max_motion_length, 1)
