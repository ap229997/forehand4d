...
class QuantizeEMAReset(nn.Module):
    ...
    def quantize(self, x, sample_codebook_temp=0., diversity=False):
        # N X C -> C X N
        k_w = self.codebook.t()
        # x: NT X C
        # NT X N
        ...
        code_idx = gumbel_sample(-distance, dim = -1, temperature = sample_codebook_temp, stochastic=True, training = self.training or diversity)
    ...
    def forward(self, x, return_idx=False, temperature=0., diversity=False):
        N, width, T = x.shape
        ...
        code_idx = self.quantize(x, temperature, diversity)
