...
from src.callbacks.loss.loss_function import compute_loss_motion
...
from src.models.latentact.model import LatentAct
...
class LatentActWrapper(GenericWrapper):
    ...
    def forward(self, inputs, targets, meta_info, mode):
        ...
        # add extra losses from VQVAE
        for k, v in pred.items():
            if 'loss_' in k:
                weight = 1.0
                if 'perplexity' in k:
                    weight = 0.0
                loss_dict[f"loss/{k.replace('loss_','')}"] = (v.mean(), weight)
        
        # remove these keys from pred since already included in loss_dict
        irrelevant_keys = ['loss_commit', 'loss_perplexity']
