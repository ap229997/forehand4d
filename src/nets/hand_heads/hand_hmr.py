...
class HandHMR(nn.Module):
    def __init__(self, feat_dim, is_rhand, n_iter, **kwargs):
        ...
        if self.is_tf_decoder:
            ...
            self.cam_init_precursor = nn.Sequential( # trying out this new variant
                nn.Linear(cam_init_dim, feat_dim),
                nn.ReLU(),
            )

        # initialization affects convergence for the below MLP
        self.cam_init = nn.Sequential( # change the name of this layer to fix initialization from pretrained ckpt issue
