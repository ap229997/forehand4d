...
def keypoint_3d_loss(pred_keypoints_3d, gt_keypoints_3d, criterion, jts_valid, return_mean=True):
    """
    ...
    return joints_loss(pred_keypoints_3d, gt_keypoints_3d, criterion, jts_valid, return_mean=return_mean)
...
def hand_kp3d_loss(pred_3d, gt_3d, criterion, jts_valid, return_mean=True, subtract_root=True):
    if subtract_root:
        pred_3d_ra = subtract_root_batch(pred_3d, root_idx=0)
        gt_3d_ra = subtract_root_batch(gt_3d, root_idx=0)
        loss_kp = keypoint_3d_loss(
            pred_3d_ra, gt_3d_ra, criterion=criterion, jts_valid=jts_valid, return_mean=return_mean,
        ) # this also subtracts the root
    else:
        loss_kp = joints_loss(
            pred_3d, gt_3d, criterion=criterion, jts_valid=jts_valid, return_mean=return_mean,
        )
    return loss_kp


def vector_loss(pred_vector, gt_vector, criterion, is_valid=None, return_mean=True):
    ...
    if is_valid.sum() == 0:
        if return_mean:
            ...
            return torch.zeros_like(dist).to(gt_vector.device)
    if is_valid is not None:
        valid_idx = is_valid.long().bool()
        # dist = dist[valid_idx] # og code
        dist = dist * is_valid[...,None]
    if return_mean:
...
def joints_loss(pred_vector, gt_vector, criterion, jts_valid, return_mean=True):
    dist = criterion(pred_vector, gt_vector)
...
def mano_loss(pred_rotmat, pred_betas, gt_rotmat, gt_betas, criterion, is_valid=None, return_mean=True):
    loss_regr_pose = vector_loss(pred_rotmat, gt_rotmat, criterion, is_valid, return_mean)
    loss_regr_betas = vector_loss(pred_betas, gt_betas, criterion, is_valid, return_mean)
    return loss_regr_pose, loss_regr_betas

def grasp_loss(pred_grasp, gt_grasp, is_valid, return_mean=True):
    loss_grasp = cross_entropy_loss(pred_grasp, gt_grasp)
    loss_grasp = loss_grasp * is_valid
    if return_mean:
        loss_grasp = loss_grasp.mean()
    return loss_grasp

def mask_loss(pred_mask, gt_mask, is_valid, return_mean=True):
    loss_grasp = bce_loss(pred_mask, gt_mask)
    loss_grasp = loss_grasp * is_valid
    if return_mean:
        loss_grasp = loss_grasp.mean()
    return loss_grasp

def render_loss(pred_mask, gt_mask, is_valid, return_mean=True):
    bz = pred_mask.shape[0]
    loss_grasp = l1_loss(pred_mask, gt_mask).view(bz, -1)
    loss_grasp = loss_grasp * is_valid[..., None]
    if return_mean:
        loss_grasp = loss_grasp.mean()
    return loss_grasp