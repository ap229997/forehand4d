...
def compute_joint3d_error(joints3d_cam_gt, joints3d_cam_pred, valid_jts):
    ...
    # print (dist.shape, joints3d_cam_gt.shape, joints3d_cam_pred.shape, valid_jts.shape, invalid_idx.shape, valid_jts.shape)
    dist[invalid_idx, :] = float("nan")
    dist = dist.cpu().numpy()
    return dist


def compute_joint3d_error_fix(joints3d_cam_gt, joints3d_cam_pred, valid_jts):
    """
    Computes the 3D joint error between ground truth and predicted joints.
    This works for both (T) and (T, J) input types.
    """
    valid_jts = valid_jts.reshape(-1)
    assert joints3d_cam_gt.shape == joints3d_cam_pred.shape
    batched = False
    if joints3d_cam_gt.ndim == 3:
        batch_dim = joints3d_cam_gt.shape[0]
        joints3d_cam_gt = joints3d_cam_gt.reshape(-1, 3)
        joints3d_cam_pred = joints3d_cam_pred.reshape(-1, 3)
        batched = True
    assert joints3d_cam_gt.shape[0] == valid_jts.shape[0]
    dist = ((joints3d_cam_gt - joints3d_cam_pred) ** 2).sum(dim=1).sqrt()
    invalid_idx = torch.nonzero((1 - valid_jts).long())
    dist[invalid_idx] = float("nan")
    if batched:
        dist = dist.reshape(batch_dim, -1)
    dist = dist.cpu().numpy()
    return dist


def compute_pixel_error(joints2d_cam_gt, joints2d_cam_pred, valid_jts):
    assert joints2d_cam_gt.shape == joints2d_cam_pred.shape
    assert joints2d_cam_gt.shape[0] == valid_jts.shape[0]
    dist = ((joints2d_cam_gt - joints2d_cam_pred) ** 2).sum(dim=2).sqrt()
    invalid_idx = torch.nonzero((1 - valid_jts).long())
    # print (dist.shape, joints2d_cam_gt.shape, joints2d_cam_pred.shape, valid_jts.shape, invalid_idx.shape, valid_jts.shape)
    dist[invalid_idx[:,0],invalid_idx[:,1]] = float("nan") # works only for 2d pixel error
    # dist[invalid_idx] = float("nan") # used previously
    dist = dist.cpu().numpy()
    return dist
