...
def color2material(mesh_color: list, metallicFactor=0.1, alphaMode="OPAQUE"):
    material = pyrender.MetallicRoughnessMaterial(
        ...
        baseColorFactor=(
            ...
            mesh_color[1] / 255.0,
            mesh_color[2] / 255.0,
            0.5, # alpha doesn't matter with 'OPAQUE' mode above
        ),
    )
    return material
...
class Renderer:
    ...
    def render_meshes_pose(
        ...
        if cam_transl is not None:
            ...
            cam_trans = None
        # og_meshes = copy.deepcopy(meshes)
        ...
        if sideview_angle is not None:
            ...
            for mesh in copy.deepcopy(meshes):
                ...
                # mesh.vertices += np.array([0, 0, -0.10]) # uncommented in the original version, check if needed
        ...
        if topdown_angle is not None:
            ...
            rot = trimesh.transformations.rotation_matrix(
                np.radians(topdown_angle), [1, 0, 0]
            )
            out_meshes = []
            for mesh in copy.deepcopy(meshes):
                mesh.vertices -= center
            ...
            # add a floor plane at y=-0.5, color gray
            # Create a thin box with 8 vertices (4 top, 4 bottom)
            floor_mesh = trimesh.Trimesh(
                vertices=[
                    # Top face vertices
                    [-50, -0.5, -50], [50, -0.5, -50], [50, -0.5, 50], [-50, -0.5, 50],
                    # Bottom face vertices  
                    [-50, -0.6, -50], [50, -0.6, -50], [50, -0.6, 50], [-50, -0.6, 50]
                ],
                faces=[
                    # Top face
                    [0, 1, 2], [0, 2, 3],
            ...
            floor_mesh.vertices += center
            floor_mesh.visual.face_colors = [128, 128, 128, 255]
            # Ensure the floor is double-sided
            floor_mesh.fix_normals()
            meshes.append(floor_mesh)
            materials.append(color2material([128, 128, 128]))
    ...
    def position_camera(self, cam_transl, K):
        ...
        cx = K[0, 2]
        cy = K[1, 2]
        # camera = pyrender.IntrinsicsCamera(fx=fx, fy=fy, cx=cx, cy=cy) # og camera
        camera = pyrender.IntrinsicsCamera(fx=fx, fy=fy, cx=cx, cy=cy, zfar=1e12) # rendering at large distances
    ...
    def reset_scene(self, K, cam_transl=None):
        if cam_transl is not None:
            cam_trans = np.copy(cam_transl)
            cam_trans[0] *= -1.0
        else:
    ...
    def render_mesh(self, og_meshes, image=None, materials=None):
        curr_meshes = copy.deepcopy(og_meshes)
        meshes = flip_meshes(curr_meshes)

        if materials is not None:
            if not isinstance(materials, list):
                bz = len(meshes)
                materials = [materials] * bz
            meshes = [
                pyrender.Mesh.from_trimesh(mesh, material=material)
                for mesh, material in zip(meshes, materials)
            ]
