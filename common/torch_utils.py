...
def to_tensor(data):
    """
    Recursively converts dictionaries, lists, tuples, and NumPy objects to PyTorch tensors.

    Args:
        data: The input data (can be dict, list, tuple, NumPy array, or scalar).

    Returns:
        A PyTorch tensor or a structure (dict) with PyTorch tensors.
    """
    if isinstance(data, dict):
        # Recursively convert each value in the dictionary
        return {key: to_tensor(value) for key, value in data.items()}
    elif isinstance(data, list):
        # Check if it's a list of strings, if so, return as is
        if all(isinstance(item, str) for item in data):
            return data
        # Convert list to a PyTorch tensor
        return torch.tensor([to_tensor(item) for item in data])
    elif isinstance(data, tuple):
        # Convert tuple to a PyTorch tensor
        return torch.tensor([to_tensor(item) for item in data])
    elif isinstance(data, np.ndarray):
        # Convert NumPy array to PyTorch tensor
        return torch.from_numpy(data)
    elif isinstance(data, (int, float, bool)):
        # Convert scalars directly to tensors
...
def reduce_tensor_dims(tensor, dims):
    """
    Reduce the tensor by combining the given dimensions into a single one.

    Args:
        tensor (torch.Tensor): The input tensor.
        dims (list or tuple): Contiguous dimensions to be reduced.
    ...
    Example:
        ...
        reduced = reduce_dim(tensor, [1, 2])
    """
    if not isinstance(tensor, torch.Tensor):
        raise TypeError("Input must be a torch.Tensor")

    if not isinstance(dims, (list, tuple)) or len(dims) == 0:
        raise ValueError("dims must be a non-empty list or tuple of dimensions")

    # Validate that dimensions are contiguous
    sorted_dims = sorted(dims)
    if not all(sorted_dims[i] + 1 == sorted_dims[i + 1] for i in range(len(sorted_dims) - 1)):
        raise ValueError("dims must be contiguous")
    
    curr_shape = tensor.shape
    new_shape = curr_shape[:sorted_dims[0]] + (-1,) + curr_shape[sorted_dims[-1] + 1:]
    tensor = tensor.reshape(new_shape)

    return tensor

    # # Permute tensor to bring the dimensions to be reduced to the end
    # all_dims = list(range(tensor.dim()))
    # permute_order = (
    #     all_dims[:sorted_dims[0]] +
    #     all_dims[sorted_dims[-1] + 1:] +
    #     all_dims[sorted_dims[0]:sorted_dims[-1] + 1]
    # )
    # tensor = tensor.permute(permute_order)

    # # Combine the reduced dimensions into one
    # new_shape = tensor.shape[:sorted_dims[0]] + (-1,) + tensor.shape[sorted_dims[-1] + 1:]
    # tensor = tensor.reshape(new_shape)

    # return tensor


def reduce_dict_dims(curr_dict, dims):
    """
    Reduce the dimensions of all tensors in the dictionary by combining the given dimensions into a single one.
    ...
    Example:
        dict = {"a": torch.rand(3, 4, 5), "b": torch.rand(2, 3, 4, 5)}
        reduced = reduce_dict_dims(dict, [1, 2])
    """
    ...
    for key, value in curr_dict.items():
        if isinstance(value, torch.Tensor):
            curr_dict.overwrite(key, reduce_tensor_dims(value, dims))

    return curr_dict


def expand_dim(tensor, curr_dim, dims):
    """
    Expand the reduced tensor into the provided dimensions.

    Args:
        tensor (torch.Tensor): The input tensor.
        curr_dim (int): The dimension to be expanded.
        dims (tuple): The target shape to expand into.
    ...
    Example:
        ...
        expanded = expand_dim(tensor, 1, (4, 5))
    """
    if not isinstance(tensor, torch.Tensor):
        raise TypeError("Input must be a torch.Tensor")
    ...
    # Expand the given dimension
    new_shape = tensor.shape[:curr_dim] + dims + tensor.shape[curr_dim + 1:]
    tensor = tensor.reshape(new_shape)

    return tensor


def expand_dict_dims(curr_dict, curr_dim, dims):
    """
    Expand the reduced dimensions of all tensors in the dictionary into the provided dimensions.
    ...
    Example:
        dict = {"a": torch.rand(3, 20), "b": torch.rand(2, 3, 20)}
        expanded = expand_dict_dims(dict, 1, (4, 5))
    """
    if not isinstance(curr_dict, dict):
        raise TypeError("Input must be a dictionary")
    ...
    for key, value in curr_dict.items():
        if isinstance(value, torch.Tensor):
            curr_dict.overwrite(key, expand_dim(value, curr_dim, dims))
