...
def add_paths(args):
    ...
    args_p = f"{args.root_dir}/{exp_key}/args.json"
    ckpt_p = f"{args.root_dir}/{exp_key}/checkpoints/last.ckpt"
    if not op.exists(ckpt_p) or DUMMY_EXP in ckpt_p:
        ckpt_p = ""
    if args.resume_ckpt != "":
        ckpt_p = args.resume_ckpt
    args.ckpt_p = ckpt_p
    args.log_dir = f"{args.root_dir}/{exp_key}"
...
def push_images(experiment, all_im_list, global_step=None, no_tqdm=False, verbose=True):
    ...
    for im in iterator:
        im_np = np.array(im["im"]).transpose(2,0,1)[:3]
        if "fig_name" in im.keys():
            experiment.add_image(im["fig_name"], im_np, global_step=global_step)
        else:
            experiment.add_image("unnamed", im_np, global_step=global_step)
    if verbose:
        toc = time.time()
