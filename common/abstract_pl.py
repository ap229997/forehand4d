...
class AbstractPL(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss_dict_unweighted = {}
        for k, v in out['loss_unweighted'].items():
            if k != 'loss':
                loss_dict_unweighted[k] = v[0].mean().view(-1).detach()
            else:
                loss_dict_unweighted[k] = v[0].mean().view(-1)
        loss_dict_unweighted.update({"total_loss": total_loss.detach(), "loss": total_loss.detach()})
        
        self.loss_dict_vec.append(loss_dict_unweighted)
        ...
        if batch_idx % log_every == 0:
            ...
            if self.args.logger == 'comet':
                ...
                if self.global_rank == 0:
                    self.log_dict(running_loss_dict, logger=True)
        if self.args.debug and self.global_step % self.args.vis_every == 0:
            print("Rendering train images")
            self.visualize_batches(self.vis_train_batches, "_train", False, only_hands='light' in self.args.method)
        return loss_dict
    ...
    def inference_epoch_end(self, out_list, postfix):
        ...
        if self.args.logger == 'comet':
            log_dict(self.experiment, loss_metric_dict, step=self.global_step,)
        elif self.args.logger == 'tensorboard':
            if self.global_rank == 0:
                self.log_dict(loss_metric_dict, logger=True)

        if not self.args.no_vis:
            print("Rendering train images")
            self.visualize_batches(self.vis_train_batches, "_train", False, only_hands='light' in self.args.method)
            print("Rendering val images")
            self.visualize_batches(self.vis_val_batches, "_val", False, only_hands='light' in self.args.method)
    ...
    def configure_optimizers(self):
        ...
        scheduler = optim.lr_scheduler.MultiStepLR(
            optimizer, self.args.lr_dec_epoch, gamma=self.args.lr_decay, verbose=False
        )
        return [optimizer], [scheduler]

    def visualize_batches(self, batches, postfix, no_tqdm=True, only_hands=False):
        im_list = []
        if self.training:
            self.eval()
        ...
        if self.args.logger == 'comet':
            ...
            self.push_images(self.logger.experiment, im_list, self.global_step)
