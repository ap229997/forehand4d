...
class ObjectTensors(nn.Module):
    def __init__(self):
        ...
        self.obj_tensors = thing.thing2dev(construct_obj_tensors(OBJECTS), "cpu") # why cpu? og: "cpu"
        self.dev = None
...
def construct_obj(object_model_p):
    ...
    top_sub_p = f"{os.environ['DOWNLOADS_DIR']}/data/arctic/data/meta/object_vtemplates/{obj_name}/top_keypoints_300.json"
    ...
    with open(parts_p, "r") as f:
        parts = np.array(json.load(f), dtype=bool) # replace np.bool by bool
    ...
    with open(f"{os.environ['DOWNLOADS_DIR']}/data/arctic/data/meta/object_meta.json", "r") as f:
...
def construct_obj_tensors(object_names):
    ...
    for k in object_names:
        object_model_p = f"{os.environ['DOWNLOADS_DIR']}/data/arctic/data/meta/object_vtemplates/%s" % (k)
